...
final class DecodeThread extends Thread {
    ...
    DecodeThread(CaptureActivityHandler CapActHandler, Vector<BarcodeFormat> decodeFormats,
    ...
    Handler getHandler() {
        try {
            ...
            ie.printStackTrace();
            ComUtils.print("DecodeThread getHandler():exception=" + ie.getMessage());
        }
        return handler;
    }
    ...
    public void run() {
        Looper.prepare();
        // 解码线程初始化解码的处理类，预览拍照的回掉会到DecodeHandler进行操作
        handler = new DecodeHandler(CapActHandler, hints);
