...
public final class CameraManager {
     ...
     *                     在初始化CaptureActivityHandler的时候开启预览startPreview()
     */
    public void openDriver(SurfaceHolder holder) throws IOException {
        ComUtils.print("CameraManager openDriver():启动相机驱动，打开相机，设置相机参数。");
        if (camera == null) {
            camera = Camera.open();
            if (camera == null) {
            ...
            //设置闪光灯，设置缩放比例:edit by wujn
            configManager.setDesiredCameraParameters(camera);

//			FlashlightManager.enableFlashlight();//delete by wujn
        }
    }
...
//		    camera.setParameters(mParameters); 
     ...
     *                开启拍照回掉和预览回掉，回掉的handler的message=R.id.decode
     */
    public void requestPreviewFrame(Handler handler, int message) {
        ComUtils.print("CameraManager requestPreviewFrame():---> 再次预览，拍照，回调...");
        if (camera != null && previewing) {
    ...
    public Rect getFramingRect() {
        ...
        if (framingRect == null) {
            ...
            // topOffset + height);
            framingRect = CoreUtils.frameRect;
        ...
        ComUtils.print("CameraManager getFramingRect():计算出的扫描二维码矩形【有效区】: " + framingRect);
        return framingRect;
    }
    ...
    public Rect getFramingRectInPreview() {
        if (framingRectInPreview == null) {
            ...
            ComUtils.print("CameraManager getFramingRectInPreview():cameraResolution:" + cameraResolution +
                    "|screenResolution:" + screenResolution + "|rect:" + rect);
        ...
        ComUtils.print("CameraManager getFramingRectInPreview():【竖屏】计算出的扫描二维码矩形在【预览上的有效区】: " + framingRectInPreview);
    ...
    public PlanarYUVLuminanceSource buildLuminanceSource(byte[] data,
        ...
        ComUtils.print("CameraManager buildLuminanceSource():===准备把图片byte[]转化为PlanarYUVLuminanceSource图片源===");

        Rect rect = getFramingRectInPreview();
        int previewFormat = configManager.getPreviewFormat();
        String previewFormatString = configManager.getPreviewFormatString();
        ComUtils.print("CameraManager buildLuminanceSource():获得预览格式code=" + previewFormat + ",预览格式str=" + previewFormatString + ",【竖屏】计算出的扫描二维码矩形在【预览上的有效区】=" + rect);
